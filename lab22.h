...
void Unit::equip(Equipment *item){
	if(equipment != NULL){
		vector<int> item_stats = equipment->getStat();
		hpmax = hpmax - item_stats[0];
		atk = atk - item_stats[1];
		def = def - item_stats[2];
	}
	vector<int> item_stats = item->getStat();
		hpmax = hpmax + item_stats[0];
		atk = atk + item_stats[1];
		def = def + item_stats[2];
		equipment = item;
